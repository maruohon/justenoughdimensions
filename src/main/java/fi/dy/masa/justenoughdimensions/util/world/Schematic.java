...
import fi.dy.masa.justenoughdimensions.util.PositionUtils;
...
public class Schematic
{
            ...
            for (int z = startZ; z < endZ; ++z)
            {
                for (int x = startX; x < endX; ++x, ++index)
                {
                    ...
                    if (te != null)
                    {
                        ...
                        {
                            JustEnoughDimensions.logger.warn("Exception while trying to store TileEntity data for block '{}' at {}",
    ...
    public boolean readFromNBT(NBTTagCompound nbt)
    {
            ...
            nbt.hasKey("Data", Constants.NBT.TAG_BYTE_ARRAY))
        {
            ...
            {
                JustEnoughDimensions.logger.error("Schematic: Mismatched block array size compared to the width/height/length, blocks: {}, W x H x L: {} x {} x {}",
            ...
            {
                JustEnoughDimensions.logger.error("Schematic: Mismatched block ID and metadata array sizes, blocks: {}, meta: {}", numBlocks, metaArr.length);
                return false;
            }

            if (this.readPalette(nbt) == false)
            {
                JustEnoughDimensions.logger.error("Schematic: Failed to read the block palette");
                return false;
            }
            ...
            if (nbt.hasKey("AddBlocks", Constants.NBT.TAG_BYTE_ARRAY))
            {
                ...
                {
                    JustEnoughDimensions.logger.error("Schematic: Add array size mismatch, blocks: {}, add: {}, expected add: {}",
            ...
            else if (nbt.hasKey("Add", Constants.NBT.TAG_BYTE_ARRAY))
            {
                ...
                JustEnoughDimensions.logger.error("Schematic: Old Schematica format detected, not currently implemented...");
                return false;
            }
        ...
        {
            JustEnoughDimensions.logger.error("Schematic: Missing block data in the schematic '{}'", this.fileName);
        }
    ...
    private boolean readPalette(NBTTagCompound nbt)
    {
        ...
        if (nbt.hasKey("SchematicaMapping", Constants.NBT.TAG_COMPOUND))
        {
            ...
            for (String key : keys)
            {
                ...
                {
                    JustEnoughDimensions.logger.error("Schematic: Invalid ID '{}' in SchematicaMapping for block '{}', max = 4095", id, key);
                    return false;
                }
                ...
                {
                    JustEnoughDimensions.logger.error("Schematic: Missing/non-existing block '{}' in SchematicaMapping", key);
                }
            }
        }
        // MCEdit2 palette
        else if (nbt.hasKey("BlockIDs", Constants.NBT.TAG_COMPOUND))
        {
            ...
            for (String idStr : keys)
            {
                ...
                {
                    JustEnoughDimensions.logger.error("Schematic: Invalid ID '{}' (not a number) in MCEdit2 palette for block '{}'", idStr, key);
                    continue;
                }

                if (id >= this.palette.length)
                {
                    JustEnoughDimensions.logger.error("Schematic: Invalid ID '{}' in MCEdit2 palette for block '{}', max = 4095", id, key);
                    return false;
                }
                ...
                {
                    JustEnoughDimensions.logger.error("Schematic: Missing/non-existing block '{}' in MCEdit2 palette", key);
                }
            }
        }
        ...
        else
        {
            for (Block block : ForgeRegistries.BLOCKS.getValuesCollection())
            {
                if (block != null)
                {
                    ...
                    {
                        JustEnoughDimensions.logger.error("Schematic: Invalid ID {} for block '{}' from the registry", id, block.getRegistryName());
                    }
                }
            }
        }
    ...
    public boolean readFromFile(File file)
    {
        if (file.exists() && file.isFile() && file.canRead())
        {
            ...
            {
                JustEnoughDimensions.logger.error("Schematic: Failed to read Schematic data from file '{}'", file.getAbsolutePath());
            }
        }
    ...
    public boolean writeToFile(File file)
    {
        ...
        {
            JustEnoughDimensions.logger.error("Schematic: Failed to write Schematic data to file '{}'", file.getAbsolutePath());
        }
